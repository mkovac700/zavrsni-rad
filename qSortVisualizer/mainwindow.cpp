...
void MainWindow::generateElements(const int &index)
{
    ...
    case 0: //nasumicno
    {
        ...
        for (int i=0; i<ui->spinBoxSize->value() ; i++ ) {
            numbers->append(QString::number(QRandomGenerator::global()->bounded(1,51)));
        }
    ...
    case 4://uniformno
    {
        ...
        int pivot = QRandomGenerator::global()->bounded(2,50); //[2,50>

        for (int i=0; i<ui->spinBoxSize->value() ; i++ ) {
            numbers->append(QString::number(QRandomGenerator::global()->bounded(pivot-1,pivot+2)));
        }
    ...
    case 6: //prilagodeno
    {
        ...
        ui->lineEditElements->setPlaceholderText(tr("Moguće je unijeti najviše 20 brojeva u rasponu 1-50 odvojenih razmakom."));

        ui->spinBoxSize->setValue(0);
...
void MainWindow::loadStatisticsGraph()
{
    ...
    //![3]
        ...
        stepsChart->setAnimationOptions(QChart::SeriesAnimations);
    //![3]

    //![4]
        QStringList stepsCategories;
        stepsCategories << tr("Broj koraka") ;
        QBarCategoryAxis *stepsAxisX = new QBarCategoryAxis();
    ...
    //![3]
        ...
        compsChart->setAnimationOptions(QChart::SeriesAnimations);
    //![3]

    //![4]
        QStringList compsCategories;
        compsCategories << tr("Broj usporedbi") ;
        QBarCategoryAxis *compsAxisX = new QBarCategoryAxis();
    ...
    //![3]
        ...
        swapsChart->setAnimationOptions(QChart::SeriesAnimations);
    //![3]

    //![4]
        QStringList swapsCategories;
        swapsCategories << tr("Broj zamjena") ;
        QBarCategoryAxis *swapsAxisX = new QBarCategoryAxis();
...
void MainWindow::on_lineEditElements_textEdited(const QString &text)
{
    ...
    for (int i = 0; i < numbers->count(); i++) {
        if(numbers->at(i).toInt() < 1 || numbers->at(i).toInt() > 50){
            isValid = false;
            break;
        }
    }
    ...
    if(!isValid){
        QMessageBox::critical(this,tr("Greška"),tr("Neispravan unos! Unos može sadržavati najviše 20 brojeva u rasponu 1-50."));
...
{
    for(int i = 0; i < 5; i++){
        if(!statisticsWorkers.at(i)->isFinished()) statisticsWorkers.at(i)->stop();
    }
